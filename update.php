...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['completed_task']) && !empty($_POST['completed_task'])) {
        // Check if the session variable 'task_entries' is set
        if (isset($_SESSION['task_entry'])) {
            // Retrieve the task entries from the session variable
            $task_entry = $_SESSION['task_entry'];

            // Loop through each checked checkbox
            foreach ($_POST['completed_task'] as $index) {
