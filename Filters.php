...
$sql = "SELECT * FROM task WHERE username = ? ORDER BY priority";
    ...
    <body style="background-color:#1b1b1b;overflow-x:auto;overflow-y:hidden;">
        <div class="front_page" style="display:flex;justify-content:space-between;">
            ...
            <br><br><br>
                ...
                <h2 style="font-family:Tahoma;margin-top:90px;margin-left:90px;color:#ceff1a;">Filter By Priority</h2>
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    $_SESSION['task_entriess'] = $task_entriess;
                    echo '<div id="task-table" style="background-color:#000;border-radius:10px;color:#fff;width:1000px;margin-top:60px;">';
                    echo '<form id="task-formm" action="updatee.php" method="post">';
                    // Initialize variable to store previous date
                    $prev_date = null;
                    
                    // Output data of each row
                    foreach ($task_entriess as $index => $task) {// Format the due date

                        // Check if the date has changed
                        if ($task["priority"] != $priority_heading) {
                        // Output heading for the new date
                        echo "<h3 style='margin-left:90px;color:#ceff1a;'>Priority " . $task["priority"] . "</h3>";
                        $priority_heading = $task["priority"];
                        }
                        ...
                        echo '<td style="width:700px;"><input type="checkbox" style="background-color:#1b1b1b;" name="completed_taskss[]" value="' . $index . '" ' . $checked . ' onchange="submitForm()">';
                        ...
                        $now = date('Y-m-d');
                        if ($dueDate < strtotime($now)) {
                            // If due date is over, set font color of due date to red
                            echo '<p style="font-family:Arial;font-size:12px;font-weight:lighter;color:red;">' . $task["due_date"] . '</p>';
                        } else {
                    ...
                    echo '</button>';
                    echo '</div>';
                    echo '<img id="fp-image" style="margin-top:30px;margin-left:400px;width:300px;height:300px;" src="filters .png">';
                ...
                <div id="task-form" style="display:none;margin-left:100px;margin-top:100px;">
                        <form action="Filter.php" method="POST" onsubmit="return validate()">   
            ...
            <script>
                function submitForm() {
                    document.getElementById("task-formm").submit();
                }
