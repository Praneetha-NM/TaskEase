...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['completed_taskss']) && !empty($_POST['completed_taskss'])) {
        // Check if the session variable 'task_entries' is set
        if (isset($_SESSION['task_entriess'])) {
            // Retrieve the task entries from the session variable
            $task_entries = $_SESSION['task_entriess'];

            // Loop through each checked checkbox
            foreach ($_POST['completed_taskss'] as $index) {
